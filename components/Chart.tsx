...
export const FinancialChart: React.FC<ChartProps> = (props) => {
  ...
  useEffect(() => { if (!chartRef.current) return; const mode = config.showCrosshair !== false ? CrosshairMode.Normal : CrosshairMode.Hidden; chartRef.current.applyOptions({ crosshair: { mode } }); if (config.showCrosshair !== false) { chartRef.current.applyOptions({ crosshair: { vertLine: { visible: true, labelVisible: true }, horzLine: { visible: true, labelVisible: true }, } }); } }, [activeToolId, config.showCrosshair]);
  ...
  useEffect(() => { if (canvasRef.current) { const isDrawingTool = !['cross'].includes(activeToolId); const isInteractive = isDrawingTool || isReplaySelecting; if (config.showCrosshair === false) { document.body.style.cursor = 'default'; canvasRef.current.style.pointerEvents = isInteractive ? 'auto' : 'none'; return; } if (isInteractive) { canvasRef.current.style.pointerEvents = 'auto'; if (activeToolId === 'brush') document.body.style.cursor = 'url(data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxNiIgaGVpZHTgxNiIgdmlld0JveD0iMCAwIDE2IDE2Ij48Y2lyY2xlIGN4PSI4IiBjeT0iOCIgcj0iMyIgZmlsbD0id2hpdGUiIHN0cm9rZT0iIzNiODJmNiIHN0cm9rZS13aWR0aD0iMSIvPjwvc3ZnPg==) 8 8, crosshair'; else document.body.style.cursor = 'crosshair'; } else { canvasRef.current.style.pointerEvents = 'none'; document.body.style.cursor = 'crosshair'; } } }, [activeToolId, isReplaySelecting, config.showCrosshair]);
  ...
  const handleContainerMouseMove = (e: React.MouseEvent) => { if (isReplaySelecting && canvasRef.current) { const rect = canvasRef.current.getBoundingClientRect(); replayMouseX.current = e.clientX - rect.left; requestDraw(); return; } if (interactionState.current.isCreating || interactionState.current.isDragging) return; if (activeToolId !== 'cross' && activeToolId !== 'cursor') return; if (chartContainerRef.current && canvasRef.current) { const rect = chartContainerRef.current.getBoundingClientRect(); const { hitHandle, hitDrawing } = getHitObject(e.clientX - rect.left, e.clientY - rect.top); if (drawingsPrimitiveRef.current) { const newHoverId = hitDrawing ? hitDrawing.id : null; if (drawingsPrimitiveRef.current._hoveredDrawingId !== newHoverId) { drawingsPrimitiveRef.current._hoveredDrawingId = newHoverId; requestDraw(); } } if (propsRef.current.config.showCrosshair === false) { document.body.style.cursor = 'default'; canvasRef.current.style.pointerEvents = (hitHandle || hitDrawing) ? 'auto' : 'none'; return; } if ((hitHandle || hitDrawing) && !areDrawingsLocked) { document.body.style.cursor = hitDrawing?.properties.locked ? 'not-allowed' : hitHandle ? 'grab' : 'pointer'; canvasRef.current.style.pointerEvents = 'auto'; } else { document.body.style.cursor = 'crosshair'; canvasRef.current.style.pointerEvents = 'none'; } } };
