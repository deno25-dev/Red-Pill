...
export const StickyNoteManager: React.FC = () => {
    ...
    const loadFiles = async () => {
        ...
        if (isTauri()) {
            try {
                // Assuming backend command exists
                // const res = await tauriAPI.listStickyNotesDirectory();
            } catch (e) {
    ...
    const handleLoad = async (filename: string) => {
        if (isTauri()) {
            try {
                // const res = await tauriAPI.loadMetadataFile('notes', filename);
    ...
    const handleDelete = async (e: React.MouseEvent, filename: string) => {
        ...
        if (isTauri()) {
            try {
                // await tauriAPI.deleteMetadataFile('notes', filename);
                loadFiles(); // Refresh
            } catch (e) {
