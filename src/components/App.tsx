...
import { debugLog, report } from '../utils/logger';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (duration > 100) {
          report("PERF", "Heavy Component Render", { ms: duration.toFixed(2), component: "App" }, 'warn');
      }
  });
  ...
  const startFileStream = useCallback(async (fileSource: File | any, fileName: string, targetTabId?: string, forceTimeframe?: Timeframe, preservedReplay?: { isReplayMode: boolean, isAdvancedReplayMode: boolean, replayGlobalTime: number | null, isReplayPlaying: boolean }) => {
      ...
      try {
          ...
          vault.rawData = currentRaw;
          vault.data = displayData;
          vault.replayIndex = replayIndex;
          vault.replayGlobalTime = preservedReplay?.replayGlobalTime ?? null;
              ...
              return currentTabs.map(t => {
                  ...
                  if (shouldUpdate) {
                      ...
                      if (t.id !== tabIdToUpdate) {
                          ...
                          if (otherVault) {
                              otherVault.rawData = currentRaw;
                              otherVault.data = displayData;
                              otherVault.replayIndex = replayIndex;
  ...
  const { currentPrice, prevPrice } = useMemo(() => {
    ...
    if (activeTab.isReplayMode || activeTab.isAdvancedReplayMode) {
        if (vault.simulatedPrice !== null) {
            ...
            const currentCandleOpen = vault.data[idx].open;
            const prevClose = idx > 0 ? vault.data[idx-1].close : currentCandleOpen;
            return { currentPrice: vault.simulatedPrice!, prevPrice: prevClose };
        } else {
           const idx = Math.min(vault.replayIndex, vault.data.length - 1);
           const price = vault.data[idx].close;
           const prev = idx > 0 ? vault.data[idx-1].close : price;
    ...
    return { currentPrice: price, prevPrice: prev };
  }, [activeTab?.isReplayMode, activeTab?.isAdvancedReplayMode, activeTabId]);

  const currentSymbolName = getBaseSymbolName(activeTab?.title || '');
  const activeDataSource = activeTab?.fileState ? (activeTab.fileState.file?.name || activeTab.fileState.path || 'Unknown Source') : (tabVault.current.get(activeTabId)?.data?.length! > 0 ? 'Mock Data' : 'None');
