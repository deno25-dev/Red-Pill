...
interface ReplayControlsProps {
  ...
  progress: number; // 0 to 100 percentage (Initial only)
  position?: { x: number; y: number };
  onHeaderMouseDown?: (e: React.MouseEvent) => void;
  isAdvancedMode?: boolean; 
  tabId?: string; // New: For event filtering
  totalCandles?: number; // New: To calculate percentage
}
...
export const ReplayControls: React.FC<ReplayControlsProps> = ({
  ...
  // Ghost-Vault Listener for smooth progress updates
  useEffect(() => {
      if (!tabId || !progressBarRef.current) return;

      const handleTick = (e: any) => {
          if (e.detail.tabId !== tabId) return;
          const { index } = e.detail; // Replay hook must emit current index
          if (index !== undefined && totalCandles > 0) {
              const pct = (index / (totalCandles - 1)) * 100;
