...
export const ChartWorkspace: React.FC<ChartWorkspaceProps> = ({ 
  ...
  onSyncTrades,
  hasUnsavedTrades,
  selectedDrawingId: propSelectedId,
  onSelectDrawing: propOnSelectDrawing
}) => {
  const tradeSourceId = tab.filePath || `${tab.title}_${tab.timeframe}`;
  const { trades: persistedTrades } = useTradePersistence(tradeSourceId);

  // Apply Inversion Engine
  useInversion(tab.config, (updates) => updateTab({ config: { ...tab.config, ...updates } }));
  ...
  // Local state fallback if not provided via props
  const [localSelectedDrawingId, setLocalSelectedDrawingId] = useState<string | null>(null);
  const selectedDrawingId = propSelectedId !== undefined ? propSelectedId : localSelectedDrawingId;
  const onSelectDrawing = propOnSelectDrawing || ((id, e) => setLocalSelectedDrawingId(id));

  // Note: Simple multi-selection logic for local use if needed, but primary is single selection via prop
  const [selectedDrawingIds, setSelectedDrawingIds] = useState<Set<string>>(new Set());

  // Sync set with single ID
  useEffect(() => {
      if (selectedDrawingId) {
          setSelectedDrawingIds(new Set([selectedDrawingId]));
      } else {
  ...
  const [defaultDrawingProperties, setDefaultDrawingProperties] = useState<DrawingProperties>({ color: COLORS.line, lineWidth: 1, lineStyle: 'solid', filled: false, backgroundColor: 'rgba(59, 130, 246, 0.1)', visible: true, locked: false, smoothing: 0 });
  ...
  useEffect(() => {
    const handleLockAll = () => { onSaveHistory?.(); const areAllCurrentlyLocked = drawings.length > 0 && drawings.every(d => d.properties.locked); onUpdateDrawings(drawings.map(d => ({ ...d, properties: { ...d.properties, locked: !areAllCurrentlyLocked } }))); };
    const handleHideAll = () => { onSaveHistory?.(); const areAllCurrentlyHidden = drawings.length > 0 && drawings.every(d => !(d.properties.visible ?? true)); onUpdateDrawings(drawings.map(d => ({ ...d, properties: { ...d.properties, visible: areAllCurrentlyHidden } }))); };
    window.addEventListener('redpill-lock-all', handleLockAll); window.addEventListener('redpill-hide-all', handleHideAll);
    return () => { window.removeEventListener('redpill-lock-all', handleLockAll); window.removeEventListener('redpill-hide-all', handleHideAll); };
  ...
  useEffect(() => { const handleKeyDown = (e: KeyboardEvent) => { if (e.key === 'Escape') { if (activeToolId && activeToolId !== 'cross') onSelectTool?.('cross'); else if (selectedDrawingId) { onSelectDrawing(null); } } }; window.addEventListener('keydown', handleKeyDown); return () => window.removeEventListener('keydown', handleKeyDown); }, [activeToolId, selectedDrawingId, onSelectTool, onSelectDrawing]);
  ...
  useEffect(() => { if (favBarRef.current && favBarPos.x === 0 && favBarPos.y === 0) { setFavBarPos({ x: window.innerWidth / 2 - favBarRef.current.clientWidth / 2, y: window.innerHeight - 150 }); } }, [favoriteTools, favBarPos.x, favBarPos.y]);
  const handleFavMouseDown = (e: React.MouseEvent) => { isDraggingFav.current = true; favDragStart.current = { x: e.clientX, y: e.clientY }; favStartPos.current = { ...favBarPos }; e.preventDefault(); e.stopPropagation(); const win = (e.view as unknown as Window) || window; const el = favBarRef.current; const handleMouseMove = (ev: MouseEvent) => { if (!isDraggingFav.current || !el) return; const dx = ev.clientX - favDragStart.current.x; const dy = ev.clientY - favDragStart.current.y; el.style.left = `${favStartPos.current.x + dx}px`; el.style.top = `${favStartPos.current.y + dy}px`; }; const handleMouseUp = (ev: MouseEvent) => { isDraggingFav.current = false; win.removeEventListener('mousemove', handleMouseMove); win.removeEventListener('mouseup', handleMouseUp); if (el) setFavBarPos({ x: favStartPos.current.x + (ev.clientX - favDragStart.current.x), y: favStartPos.current.y + (ev.clientY - favDragStart.current.y) }); }; win.addEventListener('mousemove', handleMouseMove); win.addEventListener('mouseup', handleMouseUp); };
  ...
  useEffect(() => { if ((tab.isReplayMode || tab.isAdvancedReplayMode) && replayPos.x === 0 && replayPos.y === 0) { setReplayPos({ x: window.innerWidth / 2 - 160, y: window.innerHeight - 200 }); } }, [tab.isReplayMode, tab.isAdvancedReplayMode, replayPos.x, replayPos.y]);
  const handleReplayMouseDown = (e: React.MouseEvent) => { isDraggingReplay.current = true; replayDragStart.current = { x: e.clientX, y: e.clientY }; replayStartPos.current = { ...replayPos }; e.preventDefault(); const win = (e.view as unknown as Window) || window; const handleMouseMove = (ev: MouseEvent) => { if (!isDraggingReplay.current) return; setReplayPos({ x: replayStartPos.current.x + (ev.clientX - replayDragStart.current.x), y: replayStartPos.current.y + (ev.clientY - replayDragStart.current.y) }); }; const handleMouseUp = () => { isDraggingReplay.current = false; win.removeEventListener('mousemove', handleMouseMove); win.removeEventListener('mouseup', handleMouseUp); }; win.addEventListener('mousemove', handleMouseMove); win.addEventListener('mouseup', handleMouseUp); };
  ...
  useEffect(() => { if (isLayersPanelOpen && layersPanelPos.x === 0 && layersPanelPos.y === 0) { setLayersPanelPos({ x: window.innerWidth - 320, y: 100 }); } }, [isLayersPanelOpen, layersPanelPos.x, layersPanelPos.y]);
  const handleLayersMouseDown = (e: React.MouseEvent) => { isDraggingLayers.current = true; layersDragStart.current = { x: e.clientX, y: e.clientY }; layersStartPos.current = { ...layersPanelPos }; e.preventDefault(); const win = (e.view as unknown as Window) || window; const handleMouseMove = (ev: MouseEvent) => { if (!isDraggingLayers.current) return; setLayersPanelPos({ x: layersStartPos.current.x + (ev.clientX - layersDragStart.current.x), y: layersStartPos.current.y + (ev.clientY - layersDragStart.current.y) }); }; const handleMouseUp = () => { isDraggingLayers.current = false; win.removeEventListener('mousemove', handleMouseMove); win.removeEventListener('mouseup', handleMouseUp); }; win.addEventListener('mousemove', handleMouseMove); win.addEventListener('mouseup', handleMouseUp); };
  ...
  const deleteSelectedDrawing = () => { if (selectedDrawingId) { onSaveHistory?.(); const newDrawings = drawings.filter(d => d.id !== selectedDrawingId); onUpdateDrawings(newDrawings); onSelectDrawing(null); } };
  ...
  const handleSelectDrawingWrapper = (id: string | null, e?: React.MouseEvent) => { onSelectDrawing(id, e); };
  ...
  return (
    <div ref={workspaceRef} className="flex-1 flex flex-col relative min-w-0 h-full bg-[#0f172a]">
        ...
        <div onMouseDown={handleHeaderMouseDown} style={{ left: headerPos.x, top: headerPos.y }} className="absolute z-20 bg-[#1e293b]/90 backdrop-blur-sm px-4 py-2 rounded border border-slate-700 shadow-lg flex items-center gap-4 cursor-move select-none transition-shadow hover:shadow-xl hover:ring-1 hover:ring-slate-600/50">
          ...
          <div className="relative" ref={settingsRef}><button onClick={(e) => { e.stopPropagation(); setIsChartSettingsOpen(!isChartSettingsOpen); }} className="p-1 hover:bg-[#334155] rounded text-slate-400 hover:text-white transition-colors" title="Chart Settings" onMouseDown={(e) => e.stopPropagation()}><Settings size={14} /></button>{isChartSettingsOpen && (<div className="absolute top-full right-0 mt-2 w-48 bg-[#1e293b] border border-[#334155] rounded-md shadow-xl py-1 z-50 animate-in fade-in zoom-in-95 duration-100" onMouseDown={(e) => e.stopPropagation()}><div className="px-3 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider bg-[#0f172a]/50 border-b border-[#334155]">Price Scale</div><button onClick={() => updateTab({ config: { ...tab.config, priceScaleMode: 'linear' } })} className="w-full text-left px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-[#334155] flex items-center justify-between"><span>Linear</span>{(tab.config.priceScaleMode === 'linear' || !tab.config.priceScaleMode) && <Check size={12} className="text-blue-400" />}</button><button onClick={() => updateTab({ config: { ...tab.config, priceScaleMode: 'logarithmic' } })} className="w-full text-left px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-[#334155] flex items-center justify-between"><span>Logarithmic</span>{tab.config.priceScaleMode === 'logarithmic' && <Check size={12} className="text-blue-400" />}</button><button onClick={() => updateTab({ config: { ...tab.config, priceScaleMode: 'percentage' } })} className="w-full text-left px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-[#334155] flex items-center justify-between"><span>Percentage</span>{tab.config.priceScaleMode === 'percentage' && <Check size={12} className="text-blue-400" />}</button><div className="h-px bg-[#334155] my-1 mx-2"></div><button onClick={() => updateTab({ config: { ...tab.config, autoScale: !tab.config.autoScale } })} className="w-full text-left px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-[#334155] flex items-center justify-between"><span>Auto Scale</span>{(tab.config.autoScale !== false) && <Check size={12} className="text-emerald-400" />}</button><button onClick={() => updateTab({ config: { ...tab.config, invertScale: !tab.config.invertScale } })} className="w-full text-left px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-[#334155] flex items-center justify-between"><span>Invert Scale (Alt+I)</span>{tab.config.invertScale && <Check size={12} className="text-orange-400" />}</button></div>)}</div>
          {onBackToLibrary && (<><div className="h-4 w-px bg-slate-600"></div><button onClick={onBackToLibrary} className="p-1 hover:bg-[#334155] rounded text-slate-400 hover:text-red-400 transition-colors" title="Back to Library (Closes Chart)" onMouseDown={(e) => e.stopPropagation()}><FolderIcon size={14} /></button></>)}
          {showLoadToast && (<div className="absolute top-12 left-4 z-50 bg-emerald-500/20 text-emerald-300 border border-emerald-500/50 px-3 py-1.5 rounded-full text-xs font-bold animate-in fade-in slide-in-from-top-2 flex items-center gap-2 backdrop-blur-sm shadow-lg shadow-emerald-900/20"><CheckCircle2 size={12} /><span>Drawings Loaded</span></div>)}
        </div>
        ...
        {isLayersPanelOpen && (
            <LayersPanel 
                ...
                onSelectDrawing={handleSelectDrawingWrapper} 
